...
public class PlayerController1 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        playerPivot.transform.Rotate(Vector3.right, -rotateValue.y * rotationSpeed * Time.fixedDeltaTime);

        angles = playerPivot.transform.localEulerAngles;
        if (angles.x < 300 && angles.x > 180)
        {
            playerPivot.transform.localRotation = Quaternion.Euler(300, 0, 0);
        }
        if (angles.x > 45 && angles.x < 180)
        {
            playerPivot.transform.localRotation = Quaternion.Euler(45, 0, 0);
        }
